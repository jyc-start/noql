    ...
    static get functionMappings() {
        return [
            ...
            },
            {
                ...
                parse: (parameters) => {
                    if (!$check.array(parameters))
                        throw new Error('Invalid parameters for date_add');
                    if (parameters.length < 3) {
                        throw new Error(
                            'Invalid parameters required for date_add'
                        );
                    }
                    const res = {
                        $dateAdd: {
                            startDate: parameters[0],
                            unit: this._getLiteral(parameters[1]),
                            amount: this._getLiteral(parameters[2]),
                        },
                    };
                    if (parameters[3]) {
                        res.$dateAdd.timezone = this._getLiteral(parameters[3]);
                    }
                    return res;
                },
                jsonSchemaReturnType: 'date',
            ...
            {
                name: 'date_subtract',
                aliases: ['datesubtract'],
                allowQuery: true,
                parse: (parameters) => {
                    if (!$check.array(parameters))
                        throw new Error('Invalid parameters for date_subtract');
                    if (parameters.length < 3) {
                        throw new Error(
                            'Invalid parameters required for date_subtract'
                        );
                    }
                    const res = {
                        $dateSubtract: {
                    ...
                    if (parameters[3]) {
                        res.$dateSubtract.timezone = this._getLiteral(
                            parameters[3]
                        );
                    }
                    return res;
                ...
                parse: (parameters) => {
                    ...
                    const res = {
                        $dateDiff: {
                            ...
                            unit: this._getLiteral(parameters[2]),
                        },
                    };
                    if (parameters[3]) {
                        res.$dateDiff.timezone = this._getLiteral(
                            parameters[3]
                        );
                    }
                    if (parameters[4]) {
                        res.$dateDiff.startOfWeek = this._getLiteral(
                            parameters[4]
                        );
                    }
                    return res;
            ...
            },
            {
                ...
                parse: (parameters) => {
                    return {
                        $cond: [parameters[0], parameters[1], parameters[2]],
                    };
                },
                jsonSchemaReturnType: 'string',
            },
            ...
            {
                name: 'time_to_days',
                aliases: ['time_to_days'],
                parse: (fields) => {
                  if (fields[1] === undefined) {
                    fields[1] = 'Asia/Shanghai'
                  }
                  return {
                    $dateToString: {
                      format: "%Y-%m-%d",
                      date: {
                        $toDate: {
                          $multiply: [fields[0], 1000]  // 将秒转换为毫秒
                        }
                      },
                      timezone: fields[1]
                    }
